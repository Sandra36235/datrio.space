...
function validateSession() { userEmail.textContent = "Adriana Claros · Demo"; }
...
completeOrderButton.addEventListener('click', () => {
  const total = cart.reduce((sum, item) => sum + item.price * item.quantity, 0);
  orderFeedback.textContent = `Venta de ${formatCurrency(total)} simulada. No se guardó ningún pedido ni se realizó un cobro.`;
  cart = [];
  currentOrder += 1;
  renderOrder();
});
